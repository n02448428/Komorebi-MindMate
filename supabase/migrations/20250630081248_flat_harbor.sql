/*
  # Schema update for improved session management

  1. New Features
    - Added completed flag to chat_sessions table
    - Added new indexes to improve query performance
    - Created complete_session function with proper parameter naming
    - Created get_recent_sessions function for retrieving session history

  2. Changes
    - Renamed function parameters with p_ prefix to avoid ambiguity
    - Added index for completed status on chat_sessions
*/
...
DROP FUNCTION IF EXISTS public.get_recent_sessions(integer, text);
