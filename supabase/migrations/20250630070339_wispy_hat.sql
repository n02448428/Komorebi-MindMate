...
-- Drop any existing get_user_stats functions (handle multiple signatures)
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN (
        ...
        EXECUTE 'DROP FUNCTION IF EXISTS public.get_user_stats(' || r.argtypes || ')';
    END LOOP;
END $$;
...
-- Create helper function to get user statistics with unique name
CREATE OR REPLACE FUNCTION public.get_user_statistics_v2()
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
GRANT EXECUTE ON FUNCTION public.get_user_statistics_v2 TO authenticated;