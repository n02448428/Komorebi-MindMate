/*
  # Session Management and Insights Enhancement

  1. New Columns
    - Add `completed` column to chat_sessions table for tracking session completion status

  2. Indexes
    - Add performance indexes for common query patterns
    - Index on insights for session_id and is_pinned lookups
    - Index on chat_sessions for type and completed status

  3. Functions
    - complete_session_with_insight: Complete a session and create associated insight
    - get_user_sessions_list: Retrieve recent sessions with optional filtering
    - get_user_session_stats: Get comprehensive user statistics
*/
...
DROP FUNCTION IF EXISTS public.complete_session_v2(uuid, text, text);
DROP FUNCTION IF EXISTS public.get_recent_sessions(int, text);
DROP FUNCTION IF EXISTS public.get_recent_sessions_v2(int, text);
DROP FUNCTION IF EXISTS public.get_user_stats();
DROP FUNCTION IF EXISTS public.get_user_stats(int);
DROP FUNCTION IF EXISTS public.get_user_stats(text);

-- Create function to complete a session and generate insight
CREATE OR REPLACE FUNCTION public.complete_session_with_insight(
  p_session_id uuid, 
...
CREATE OR REPLACE FUNCTION public.get_user_sessions_list(
...
GRANT EXECUTE ON FUNCTION public.complete_session_with_insight TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_user_sessions_list TO authenticated;
